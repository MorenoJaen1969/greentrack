...
use \Exception;
class address_clasController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "address_clasController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/address_clas/';
    ...
    public function listaraddress_clasControlador($dato_ori)
	{
		$pagina_address_clas = $dato_ori[0];
		...
		$pagina = (isset($pagina_address_clas) && $pagina_address_clas > 0) ? (int) $pagina_address_clas : 1;
        ...
        $orden_r = "address_clas";

        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "address_clas LIKE '%$busqueda%'";
            ...
            // Consulta principal: lista todos los address_clas con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM address_clas 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_address_clas = '/address_clas/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/address_clasVista/address_clas/" . $rows['id_address_clas'] . $ruta_address_clas;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_address_clas']) . '</td>
                            <td>' . $rows['address_clas'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_address_clas'] . '" class="button is-link is-rounded is-small" id="a_id_address_clas" name="a_id_address_clas"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/address_clasAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_address_clas" value="eliminar">
                                    <input type="hidden" name="id_address_clas" id="id_address_clas" value="' . $rows['id_address_clas'] . '">
    ...
    public function crear_registro($address_clas)
    {
        try {
            $address_clas = $this->limpiarCadena($address_clas);

            $datos = [
                ['campo_nombre' => 'address_clas', 'campo_marcador' => ':address_clas', 'campo_valor' => $address_clas],
            ];

            $id_address_clas = $this->guardarDatos('address_clas', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_address_clas}");

            if ($id_address_clas) {
                $this->log("Nuevo registro de address_clas creado: " . $address_clas);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de address_clas: " . $address_clas, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de address_clas: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_address_clas()
    {
        $sql = 'SELECT id_address_clas, address_clas
            FROM address_clas';
