...
class proveedoresController extends mainModel
{
	...
	public function __construct()
	{
		...
		$nom_controlador = "proveedoresController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/proveedores/';
		...
		$this->id_status_activo = 50;
		$this->id_status_inactivo = 51;
		$this->id_status_eliminado = 52;
	...
	public function listarproveedoresControlador($dato_ori)
	{
		$pagina_proveedores = $dato_ori[0];
		...
		$pagina = (isset($pagina_proveedores) && $pagina_proveedores > 0) ? (int) $pagina_proveedores : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            ...
            $consulta_total = "SELECT COUNT(c.id_proveedor) AS total 
                    FROM proveedores AS c
					LEFT JOIN status_all AS s ON s.id_status = c.id_status
					WHERE (" . $campos_filtro . ") AND " . $where;

		} else {
            // Consulta principal: lista todos los proveedores con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM proveedores c
					LEFT JOIN status_all AS s ON s.id_status = c.id_status
					WHERE " . $where . 
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_proveedores = '/proveedores/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/proveedoresVista/proveedores/" . $rows['id_proveedor'] . $ruta_proveedores;
				$total_s = 1;
				...
				$tabla .= '
                        <tr class="has-text-centered table-row">
                            ...
                            <td>' . $rows['status'] . '</td>
                            <td>
                                <a href="' . $ruta_destino . $rows['id_proveedor'] . '" 
							...
							<td>
								<button type="button" 
										class="button is-danger is-rounded is-small btn-eliminar-proveedores"
										data-id="' . $rows['id_proveedor'] . '"
										data-pagina="' . $pagina . '"
		...
		if (count((array)$datos) > 0) {
			...
			try {
				...
				$condicion = [
					...
					'condicion_operador' => '=',
					'condicion_marcador' => ':id_proveedor',
					'condicion_valor' => $id_proveedor
				];

				$this->actualizarDatos('proveedores', $datos, $condicion);
	...
	public function proveedorCompleto_proveedor($id_proveedor){
