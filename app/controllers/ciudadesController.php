...
use \Exception;
class ciudadesController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$this->log_path = APP_R_PROY . 'app/logs/ciudades/';
    ...
    public function listarciudadesControlador($dato_ori)
	{
		$pagina_ciudades = $dato_ori[0];
		...
		$pagina = (isset($pagina_ciudades) && $pagina_ciudades > 0) ? (int) $pagina_ciudades : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "ciudades LIKE '%$busqueda%'";
            ...
            $consulta_total = "SELECT COUNT(id_ciudades) AS total 
                    FROM ciudades 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los ciudades con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM ciudades 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_ciudades = '/ciudades/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/ciudadesVista/ciudades/" . $rows['id_ciudades'] . $ruta_ciudades;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_ciudades']) . '</td>
                            <td>' . $rows['ciudades'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_ciudades'] . '" class="button is-link is-rounded is-small" id="a_id_ciudades" name="a_id_ciudades"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/ciudadesAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_ciudades" value="eliminar">
                                    <input type="hidden" name="id_ciudades" id="id_ciudades" value="' . $rows['id_ciudades'] . '">
    ...
    public function crear_registro($ciudades)
    {
        try {
            $ciudades = $this->limpiarCadena($ciudades);

            $datos = [
                ['campo_nombre' => 'ciudades', 'campo_marcador' => ':ciudades', 'campo_valor' => $ciudades],
            ];

            $id_ciudades = $this->guardarDatos('ciudades', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_ciudades}");

            if ($id_ciudades) {
                $this->log("Nuevo registro de ciudades creado: " . $ciudades);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de ciudades: " . $ciudades, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de ciudades: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_ciudades($id_condado)
    {
        $sql = 'SELECT id_ciudad, nombre
            FROM ciudades
            WHERE id_condado = :v_id_condado';
