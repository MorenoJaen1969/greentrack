...
use \Exception;
class frecuencia_pagoController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "frecuencia_pagoController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/frecuencia_pago/';
    ...
    public function listarfrecuencia_pagoControlador($dato_ori)
	{
		$pagina_frecuencia_pago = $dato_ori[0];
		...
		$pagina = (isset($pagina_frecuencia_pago) && $pagina_frecuencia_pago > 0) ? (int) $pagina_frecuencia_pago : 1;
        ...
        $orden_r = "frecuencia_pago";

        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "frecuencia_pago LIKE '%$busqueda%'";

            $consulta_datos = "SELECT " . $campos . " 
                    FROM frecuencia_pago 
                    WHERE " . $campos_filtro . "
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;

            $consulta_total = "SELECT COUNT(id_frecuencia_pago) AS total 
                    FROM frecuencia_pago 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los frecuencia_pago con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM frecuencia_pago 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_frecuencia_pago = '/frecuencia_pago/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/frecuencia_pagoVista/frecuencia_pago/" . $rows['id_frecuencia_pago'] . $ruta_frecuencia_pago;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_frecuencia_pago']) . '</td>
                            <td>' . $rows['frecuencia_pago'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_frecuencia_pago'] . '" class="button is-link is-rounded is-small" id="a_id_frecuencia_pago" name="a_id_frecuencia_pago"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/frecuencia_pagoAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_frecuencia_pago" value="eliminar">
                                    <input type="hidden" name="id_frecuencia_pago" id="id_frecuencia_pago" value="' . $rows['id_frecuencia_pago'] . '">
    ...
    public function crear_registro($frecuencia_pago)
    {
        try {
            $frecuencia_pago = $this->limpiarCadena($frecuencia_pago);

            $datos = [
                ['campo_nombre' => 'frecuencia_pago', 'campo_marcador' => ':frecuencia_pago', 'campo_valor' => $frecuencia_pago],
            ];

            $id_frecuencia_pago = $this->guardarDatos('frecuencia_pago', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_frecuencia_pago}");

            if ($id_frecuencia_pago) {
                $this->log("Nuevo registro de frecuencia_pago creado: " . $frecuencia_pago);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de frecuencia_pago: " . $frecuencia_pago, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de frecuencia_pago: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_frecuencia_pago()
    {
        $sql = 'SELECT id_frecuencia_pago, descripcion
            FROM frecuencia_pago';
