...
use \Exception;
class address_typeController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "address_typeController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/address_type/';
    ...
    public function listaraddress_typeControlador($dato_ori)
	{
		$pagina_address_type = $dato_ori[0];
		...
		$pagina = (isset($pagina_address_type) && $pagina_address_type > 0) ? (int) $pagina_address_type : 1;
        ...
        $orden_r = "address_type";

        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "address_type LIKE '%$busqueda%'";
            ...
            $consulta_total = "SELECT COUNT(id_address_clas) AS total 
                    FROM address_type 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los address_type con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM address_type 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
            ...
            $consulta_total = "SELECT COUNT(id_address_type) AS total 
                    FROM address_type"; 
        }
				...
				<table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
					<thead class="cabecera">
						<tr>
							...
							<th class="has-text-centered">Address Type</th>
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_address_type = '/address_type/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/address_typeVista/address_type/" . $rows['id_address_type'] . $ruta_address_type;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_address_type']) . '</td>
                            <td>' . $rows['address_type'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_address_type'] . '" class="button is-link is-rounded is-small" id="a_id_address_type" name="a_id_address_type"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/address_typeAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_address_type" value="eliminar">
                                    <input type="hidden" name="id_address_type" id="id_address_type" value="' . $rows['id_address_type'] . '">
    ...
    public function crear_registro($address_type)
    {
        try {
            $address_type = $this->limpiarCadena($address_type);

            $datos = [
                ['campo_nombre' => 'address_type', 'campo_marcador' => ':address_type', 'campo_valor' => $address_type],
            ];

            $id_address_type = $this->guardarDatos('address_type', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_address_type}");

            if ($id_address_type) {
                $this->log("Nuevo registro de address_type creado: " . $address_type);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                    'texto' => 'The new address type has been successfully registered.'
                ]);
            } else {
                $this->log("Error al crear el nuevo registro de address_type: " . $address_type, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
                    'texto' => 'There was an error registering the new address type. Please try again.'
                ]);
            }
        } catch (Exception $e) {
            $this->log("Excepción al crear el nuevo registro de address_type: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
                'texto' => 'An exception occurred while registering the new address type: ' . $e->getMessage()
            ]);
        }
    }

    public function consultar_address_type($id_address_clas)
    {
        $sql = 'SELECT id_address_type, address_type
            FROM address_type
            WHERE id_address_clas = :v_id_address_clas';
        
        $param = [
            ':v_id_address_clas' => $id_address_clas
        ];
