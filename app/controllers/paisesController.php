...
use \Exception;
class paisesController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "paisesController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/paises/';
    ...
    public function listarpaisesControlador($dato_ori)
	{
		$pagina_paises = $dato_ori[0];
		...
		$pagina = (isset($pagina_paises) && $pagina_paises > 0) ? (int) $pagina_paises : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "paises LIKE '%$busqueda%'";
            ...
            $consulta_total = "SELECT COUNT(id_paises) AS total 
                    FROM paises 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los paises con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM paises 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_paises = '/paises/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/paisesVista/paises/" . $rows['id_paises'] . $ruta_paises;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_paises']) . '</td>
                            <td>' . $rows['paises'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_paises'] . '" class="button is-link is-rounded is-small" id="a_id_paises" name="a_id_paises"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/paisesAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_paises" value="eliminar">
                                    <input type="hidden" name="id_paises" id="id_paises" value="' . $rows['id_paises'] . '">
    ...
    public function crear_registro($paises)
    {
        try {
            $paises = $this->limpiarCadena($paises);

            $datos = [
                ['campo_nombre' => 'paises', 'campo_marcador' => ':paises', 'campo_valor' => $paises],
            ];

            $id_paises = $this->guardarDatos('paises', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_paises}");

            if ($id_paises) {
                $this->log("Nuevo registro de paises creado: " . $paises);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de paises: " . $paises, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de paises: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
