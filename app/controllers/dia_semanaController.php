...
use \Exception;
class dia_semanaController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "dia_semanaController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/dia_semana/';
    ...
    public function listardia_semanaControlador($dato_ori)
	{
		$pagina_dia_semana = $dato_ori[0];
		...
		$pagina = (isset($pagina_dia_semana) && $pagina_dia_semana > 0) ? (int) $pagina_dia_semana : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "dia_semana LIKE '%$busqueda%'";
            ...
            $consulta_total = "SELECT COUNT(id_dia_semana) AS total 
                    FROM dia_semana 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los dia_semana con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM dia_semana 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_dia_semana = '/dia_semana/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/dia_semanaVista/dia_semana/" . $rows['id_dia_semana'] . $ruta_dia_semana;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_dia_semana']) . '</td>
                            <td>' . $rows['dia_semana'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_dia_semana'] . '" class="button is-link is-rounded is-small" id="a_id_dia_semana" name="a_id_dia_semana"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/dia_semanaAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_dia_semana" value="eliminar">
                                    <input type="hidden" name="id_dia_semana" id="id_dia_semana" value="' . $rows['id_dia_semana'] . '">
    ...
    public function crear_registro($dia_semana)
    {
        try {
            $dia_semana = $this->limpiarCadena($dia_semana);

            $datos = [
                ['campo_nombre' => 'dia_semana', 'campo_marcador' => ':dia_semana', 'campo_valor' => $dia_semana],
            ];

            $id_dia_semana = $this->guardarDatos('dia_semana', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_dia_semana}");

            if ($id_dia_semana) {
                $this->log("Nuevo registro de dia_semana creado: " . $dia_semana);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de dia_semana: " . $dia_semana, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de dia_semana: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_dia_semana()
    {
        $sql = 'SELECT id_dia_semana, dia_ingles 
            FROM dias_semana';
