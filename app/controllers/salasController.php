...
use \Exception;
class salasController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "salasController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/salas/';
    ...
    public function listarsalasControlador($dato_ori)
	{
		$pagina_salas = $dato_ori[0];
		...
		$pagina = (isset($pagina_salas) && $pagina_salas > 0) ? (int) $pagina_salas : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "chat_salas LIKE '%$busqueda%'";
            ...
            // Consulta principal: lista todos los salas con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM chat_salas 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
				...
				<table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
					<thead class="cabecera">
						<tr>
							...
							<th class="has-text-centered">Chat Room</th>
							<th class="has-text-centered">Descriptions</th>
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_salas = '/salas/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/salasVista/salas/" . $rows['id_salas'] . $ruta_salas;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id']) . '</td>
                            <td>' . $rows['nombre'] . '</td>
                            <td>' . $rows['descripcion'] . '</td>
                            <td>' . ($rows['activo'] ? 'Active' : 'Inactive') . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_salas'] . '" class="button is-link is-rounded is-small" id="a_id_salas" name="a_id_salas"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/salasAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_salas" value="eliminar">
                                    <input type="hidden" name="id_salas" id="id_salas" value="' . $rows['id_salas'] . '">
    ...
    public function crear_registro($salas)
    {
        try {
            $salas = $this->limpiarCadena($salas);

            $datos = [
                ['campo_nombre' => 'salas', 'campo_marcador' => ':salas', 'campo_valor' => $salas],
            ];

            $id_salas = $this->guardarDatos('salas', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_salas}");

            if ($id_salas) {
                $this->log("Nuevo registro de salas creado: " . $salas);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de salas: " . $salas, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de salas: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_salas()
    {
        $sql = 'SELECT id_salas, salas
            FROM salas';
