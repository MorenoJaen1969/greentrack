...
use \Exception;
class frecuencia_servicioController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "frecuencia_servicioController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/frecuencia_servicio/';
    ...
    public function listarfrecuencia_servicioControlador($dato_ori)
	{
		$pagina_frecuencia_servicio = $dato_ori[0];
		...
		$pagina = (isset($pagina_frecuencia_servicio) && $pagina_frecuencia_servicio > 0) ? (int) $pagina_frecuencia_servicio : 1;
        ...
        $orden_r = "frecuencia_servicio";

        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "frecuencia_servicio LIKE '%$busqueda%'";

            $consulta_datos = "SELECT " . $campos . " 
                    FROM frecuencia_servicio 
                    WHERE " . $campos_filtro . "
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;

            $consulta_total = "SELECT COUNT(id_frecuencia_servicio) AS total 
                    FROM frecuencia_servicio 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los frecuencia_servicio con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM frecuencia_servicio 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_frecuencia_servicio = '/frecuencia_servicio/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/frecuencia_servicioVista/frecuencia_servicio/" . $rows['id_frecuencia_servicio'] . $ruta_frecuencia_servicio;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_frecuencia_servicio']) . '</td>
                            <td>' . $rows['frecuencia_servicio'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_frecuencia_servicio'] . '" class="button is-link is-rounded is-small" id="a_id_frecuencia_servicio" name="a_id_frecuencia_servicio"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/frecuencia_servicioAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_frecuencia_servicio" value="eliminar">
                                    <input type="hidden" name="id_frecuencia_servicio" id="id_frecuencia_servicio" value="' . $rows['id_frecuencia_servicio'] . '">
    ...
    public function crear_registro($frecuencia_servicio)
    {
        try {
            $frecuencia_servicio = $this->limpiarCadena($frecuencia_servicio);

            $datos = [
                ['campo_nombre' => 'frecuencia_servicio', 'campo_marcador' => ':frecuencia_servicio', 'campo_valor' => $frecuencia_servicio],
            ];

            $id_frecuencia_servicio = $this->guardarDatos('frecuencia_servicio', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_frecuencia_servicio}");

            if ($id_frecuencia_servicio) {
                $this->log("Nuevo registro de frecuencia_servicio creado: " . $frecuencia_servicio);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de frecuencia_servicio: " . $frecuencia_servicio, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de frecuencia_servicio: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_frecuencia_servicio()
    {
        $sql = 'SELECT id_frecuencia_servicio, concepto
            FROM frecuencia_servicio';
