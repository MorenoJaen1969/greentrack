...
use \Exception;
class condadosController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "condadosController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/condados/';
    ...
    public function listarcondadosControlador($dato_ori)
	{
		$pagina_condados = $dato_ori[0];
		...
		$pagina = (isset($pagina_condados) && $pagina_condados > 0) ? (int) $pagina_condados : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "condados LIKE '%$busqueda%'";
            ...
            $consulta_total = "SELECT COUNT(id_condados) AS total 
                    FROM condados 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los condados con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM condados 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_condados = '/condados/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/condadosVista/condados/" . $rows['id_condados'] . $ruta_condados;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_condados']) . '</td>
                            <td>' . $rows['condados'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_condados'] . '" class="button is-link is-rounded is-small" id="a_id_condados" name="a_id_condados"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/condadosAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_condados" value="eliminar">
                                    <input type="hidden" name="id_condados" id="id_condados" value="' . $rows['id_condados'] . '">
    ...
    public function crear_registro($condados)
    {
        try {
            $condados = $this->limpiarCadena($condados);

            $datos = [
                ['campo_nombre' => 'condados', 'campo_marcador' => ':condados', 'campo_valor' => $condados],
            ];

            $id_condados = $this->guardarDatos('condados', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_condados}");

            if ($id_condados) {
                $this->log("Nuevo registro de condados creado: " . $condados);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de condados: " . $condados, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de condados: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_condados($id_ciudad)
    {
        $sql = 'SELECT id_condado, nombre
            FROM condados
            WHERE id_estado = :v_id_estado';
        $param = [
            ':v_id_estado' => $id_ciudad
        ];
