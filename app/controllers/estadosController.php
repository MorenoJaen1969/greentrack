...
use \Exception;
class estadosController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "estadosController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/estados/';
    ...
    public function listarestadosControlador($dato_ori)
	{
		$pagina_estados = $dato_ori[0];
		...
		$pagina = (isset($pagina_estados) && $pagina_estados > 0) ? (int) $pagina_estados : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "estados LIKE '%$busqueda%'";
            ...
            $consulta_total = "SELECT COUNT(id_estados) AS total 
                    FROM estados 
                    WHERE " . $campos_filtro; 

        } else {
            // Consulta principal: lista todos los estados con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM estados 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_estados = '/estados/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/estadosVista/estados/" . $rows['id_estados'] . $ruta_estados;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_estados']) . '</td>
                            <td>' . $rows['estados'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_estados'] . '" class="button is-link is-rounded is-small" id="a_id_estados" name="a_id_estados"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/estadosAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_estados" value="eliminar">
                                    <input type="hidden" name="id_estados" id="id_estados" value="' . $rows['id_estados'] . '">
    ...
    public function crear_registro($estados)
    {
        try {
            $estados = $this->limpiarCadena($estados);

            $datos = [
                ['campo_nombre' => 'estados', 'campo_marcador' => ':estados', 'campo_valor' => $estados],
            ];

            $id_estados = $this->guardarDatos('estados', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_estados}");

            if ($id_estados) {
                $this->log("Nuevo registro de estados creado: " . $estados);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de estados: " . $estados, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de estados: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
    ...
    public function consultar_estados($id_pais)
    {
