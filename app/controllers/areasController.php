...
use \Exception;
class areasController extends mainModel
{
	private $log_path;
	private $logFile;
	...
	public function __construct()
	{
		...
		$nom_controlador = "areasController";
		// ____________________________________________________________________

		$this->log_path = APP_R_PROY . 'app/logs/areas/';
    ...
    public function listarareasControlador($dato_ori)
	{
		$pagina_areas = $dato_ori[0];
		...
		$pagina = (isset($pagina_areas) && $pagina_areas > 0) ? (int) $pagina_areas : 1;
        ...
        if (isset($busqueda) && $busqueda != "") {
            $campos_filtro = "areas LIKE '%$busqueda%'";
            ...
            // Consulta principal: lista todos los areas con tiempo restante
            $consulta_datos = "SELECT " . $campos . " 
                    FROM areas 
                    ORDER BY " . $orden_r . "
                    LIMIT " . $inicio . ", " . $registros;
            ...
            $consulta_total = "SELECT COUNT(id_area) AS total 
                    FROM areas"; 
        }
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            foreach ($datos as $rows) {
                ...
                $ruta_areas = '/areas/' . $pagina . '/';
                $ruta_destino = RUTA_APP . "/areasVista/areas/" . $rows['id_area'] . $ruta_areas;

                $tabla .= '
                        <tr class="has-text-centered table-row">
                            <td>' . $contador . '</td>
                            <td>' . $this->ceros($rows['id_area']) . '</td>
                            <td>' . $rows['areas'] . '</td>
                            <td>
                                <a href="' . $ruta_destino. '/' . $rows['id_area'] . '" class="button is-link is-rounded is-small" id="a_id_area" name="a_id_area"><span class="fas fa-eye"></a>
                            </td>
                            <td>
                                <form class="FormularioAjax" action="' . APP_URL . 'app/ajax/areasAjax.php" method="POST" autocomplete="off" >
                                    <input type="hidden" name="modulo_areas" value="eliminar">
                                    <input type="hidden" name="id_area" id="id_area" value="' . $rows['id_area'] . '">
    ...
    public function crear_registro($areas)
    {
        try {
            $areas = $this->limpiarCadena($areas);

            $datos = [
                ['campo_nombre' => 'areas', 'campo_marcador' => ':areas', 'campo_valor' => $areas],
            ];

            $id_area = $this->guardarDatos('areas', $datos);
            $this->log("Clasificacion insertado: Clasificacion = {$id_area}");

            if ($id_area) {
                $this->log("Nuevo registro de areas creado: " . $areas);
                echo json_encode([
                    'tipo' => 'success',
                    'titulo' => 'Success',
                ...
                $this->log("Error al crear el nuevo registro de areas: " . $areas, true);
                echo json_encode([
                    'tipo' => 'error',
                    'titulo' => 'Error',
            ...
            $this->log("Excepción al crear el nuevo registro de areas: " . $e->getMessage(), true);
            echo json_encode([
                'tipo' => 'error',
                'titulo' => 'Exception',
