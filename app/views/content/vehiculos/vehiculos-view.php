...
$pagina_vehiculos = $pagina;

$proceso_actual = "vehiculos";
$busqueda = "";
...
$ruta_vehiculosnew = "vehiculosNew";
...
if (!isset($_SESSION['nav_vehiculos'])) {
    ...
    if (isset($_SESSION['nav_vehiculos']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_vehiculos']['registrosPorPagina'])) {
            $_SESSION['nav_vehiculos']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_vehiculos']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_vehiculos']['orden'])) {
        $f_orden = $_SESSION['nav_vehiculos']['orden'];

        $ordenStr = $_SESSION['nav_vehiculos']['orden'];
    ...
    if (isset($_SESSION['nav_vehiculos']['pagina_vehiculos'])) {
        if (!is_numeric($_SESSION['nav_vehiculos']['pagina_vehiculos'])) {
            $_SESSION['nav_vehiculos']['pagina_vehiculos'] = $pagina_vehiculos;
        }
    } else {
        $_SESSION['nav_vehiculos'] = [
            'pagina_vehiculos' => $pagina_vehiculos
...
$no_hacer = true;
$ruta_vehiculos_ajax = APP_URL . "/app/ajax/vehiculosAjax.php";
    ...
    <div class="container pb-1 pt-1">
        <div id="datos_act" name="datos_act">
            <?php
                ...
                echo $vehiculos->listarvehiculosControlador($param_datos);
            ?>
        </div>
    </div>
</main>
...
window.CONFIG_BUSQUEDA = <?php echo json_encode([
    'modulo' => $proceso_actual ?? 'vehiculos',
    'pagina' => $pagina_vehiculos ?? 1,
...
<script type="text/javascript">
    ...
    // === Eliminación segura de vehiculos con suiteConfirm ===
    document.addEventListener('click', async function(e) {
        const btn = e.target.closest('.btn-eliminar-vehiculos');
        if (!btn) return;
        ...
        try {
            const res = await fetch('/app/ajax/vehiculosAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_vehiculos: 'eliminar',
            ...
            if (data.success) {
                ...
                recargarTablavehiculos(pagina, registrosPorPagina, urlOrigen, valor);
    ...
    async function recargarTablavehiculos(pagina, registrosPorPagina, urlOrigen, busqueda = '') {
        try {
            const res = await fetch('/app/ajax/vehiculosAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_vehiculos: 'listar_tabla',
                    pagina: pagina,
            ...
            const wrapper = document.getElementById('tabla-vehiculos-wrapper');
            if (wrapper) {
                wrapper.outerHTML = html;
            } else {
                console.error('❌ No se encontró #tabla-vehiculos-wrapper en el DOM');
            }
        } catch (err) {
            console.error('Error al recargar tabla de vehiculos:', err);
