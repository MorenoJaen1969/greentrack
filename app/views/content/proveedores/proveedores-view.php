...
$pagina_proveedores = $pagina;

$proceso_actual = "proveedores";
$busqueda = "";
...
$ruta_proveedoresnew = "proveedoresNew";
...
if (!isset($_SESSION['nav_proveedores'])) {
    ...
    if (isset($_SESSION['nav_proveedores']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_proveedores']['registrosPorPagina'])) {
            $_SESSION['nav_proveedores']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_proveedores']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_proveedores']['orden'])) {
        $f_orden = $_SESSION['nav_proveedores']['orden'];

        $ordenStr = $_SESSION['nav_proveedores']['orden'];
    ...
    if (isset($_SESSION['nav_proveedores']['pagina_proveedores'])) {
        if (!is_numeric($_SESSION['nav_proveedores']['pagina_proveedores'])) {
            $_SESSION['nav_proveedores']['pagina_proveedores'] = $pagina_proveedores;
        }
    } else {
        $_SESSION['nav_proveedores'] = [
            'pagina_proveedores' => $pagina_proveedores
...
$no_hacer = true;
$ruta_proveedores_ajax = APP_URL . "/app/ajax/proveedoresAjax.php";
...
window.CONFIG_BUSQUEDA = <?php echo json_encode([
    'modulo' => $proceso_actual ?? 'proveedores',
    'pagina' => $pagina_proveedores ?? 1,
    'registrosPorPagina' => $registrosPorPagina ?? 10,
    'url' => $url[0] ?? 'proveedores',
    'orden' => $orden ?? '#',
...
<script type="text/javascript">
    ...
    // === Eliminación segura de proveedores con suiteConfirm ===
    document.addEventListener('click', async function(e) {
        const btn = e.target.closest('.btn-eliminar-proveedores');
        if (!btn) return;
        ...
        try {
            const res = await fetch('/app/ajax/proveedoresAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_proveedores: 'eliminar',
            ...
            if (data.success) {
                ...
                recargarTablaproveedores(pagina, registrosPorPagina, urlOrigen, valor);
    ...
    async function recargarTablaproveedores(pagina, registrosPorPagina, urlOrigen, busqueda = '') {
        try {
            const res = await fetch('/app/ajax/proveedoresAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_proveedores: 'listar_tabla',
                    pagina: pagina,
            ...
            if (wrapper) {
                ...
                console.error('❌ No se encontró #tabla-proveedores-wrapper en el DOM');
            }
        } catch (err) {
            console.error('Error al recargar tabla de proveedores:', err);
