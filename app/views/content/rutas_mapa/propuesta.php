...
let rutaActual = null;
let zonasData = new Map();

// LayerGroups
let capaZonas = null;
let capaDireccionesRuta = null;
let capaDireccionesLibres = null;
let capaTodasDirecciones = null;
let marcadorHQ = null;
...
function inicializarMapa() {
    ...
    const btnAddresses = L.control({position: 'topleft'});
...
async function cargarTodasZonas() {
    try {
        const response = await fetch(RUTA_AJAX, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({modulo_rutas: 'listar_todas_zonas_completo'}) // Nuevo endpoint con info geográfica
        });
        const data = await response.json();
                ...
                const popupContent = `
                    <div style="min-width: 220px; font-family: Arial, sans-serif;">
                        ...
                        <div style="margin-top: 10px; padding-top: 8px; border-top: 1px solid #ecf0f1; font-size: 0.8em; color: #7f8c8d; font-family: monospace;">
                            <div style="margin-bottom: 3px;"><b>SW:</b> ${parseFloat(zona.lat_sw).toFixed(6)}, ${parseFloat(zona.lng_sw).toFixed(6)}</div>
                            <div><b>NE:</b> ${parseFloat(zona.lat_ne).toFixed(6)}, ${parseFloat(zona.lng_ne).toFixed(6)}</div>
                        </div>
                    </div>
                `;
                    ...
                    rect.bindPopup(`
                        <div style="min-width: 200px;">
                            ...
                            <div style="margin-top: 8px; font-size: 0.8em; color: #7f8c8d; font-family: monospace;">
                                SW: ${zona.lat_sw}, ${zona.lng_sw}<br>
                                NE: ${zona.lat_ne}, ${zona.lng_ne}
                            </div>
                        </div>
                    `);
...
async function cargarTodasDireccionesAddresses() {
    try {
        ...
        const response = await fetch(RUTA_AJAX, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({modulo_rutas: 'listar_todas_direcciones_contrato'})
        });
        const data = await response.json();
...
async function seleccionarRutaExistente(idRuta) {
    ...
    try {
        ...
        if (data.success) {
            cambiarEstado(ESTADOS.RUTA_VER, {ruta: data.data});
        } else {
    ...
    if (estadoActual === ESTADOS.RUTA_EDITAR) {
        lista.querySelectorAll('.btn-eliminar-dir').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (confirm('Remove this address from the route?\n\nThis action cannot be undone.')) {
...
async function abrirModalReordenamiento() {
    ...
    if (todasDirs.length === 0) {
        alert('No addresses to reorder');
        return;
    }
    
    if (todasDirs.length === 1) {
        alert('Need at least 2 addresses to reorder');
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            alert('Route order updated successfully!');
...
async function seleccionarZonaParaNuevaRuta(layer) {
    ...
    try {
        ...
        if (totalDisponibles === 0) {
            rutaEnConstruccion.zonas.set(idZona, {direcciones: new Set()});
            layer.setStyle({color: '#e74c3c', weight: 3, fillOpacity: 0.3});
            actualizarListaZonasSeleccionadas();
            return;
        }
        ...
        if (seleccionadas !== null) {
            rutaEnConstruccion.zonas.set(idZona, {direcciones: new Set(seleccionadas)});
            layer.setStyle({color: '#e74c3c', weight: 3, fillOpacity: 0.3});
            actualizarListaZonasSeleccionadas();
        }
        ...
        alert('Could not load zone addresses');
    }
}
...
async function abrirModalZonaNueva(layer) {
    ...
    try {
        ...
        if (direcciones.libres.length === 0 && direcciones.en_otra_ruta.length === 0) {
            if (confirm(`Zone "${nombreZona}" has no available addresses.\n\nAdd it for navigation only?`)) {
...
async function abrirModalZonaEdicion(layer) {
    ...
    try {
        ...
        const direcciones = data.success ? data.data : {libres: [], en_otra_ruta: [], en_ruta_actual: []};
        const resultado = await abrirModalEdicionZona(direcciones, nombreZona, idsEnRuta);
...
function abrirModalEdicionZona(direcciones, nombreZona, idsEnRutaActual) {
    return new Promise((resolve) => {
        const overlay = document.createElement('div');
        overlay.style.cssText = `
            position: fixed; top: 0; left: 0; width: 100%; height: 100%;
...
// ============================================
// ACCIONES DE GUARDADO Y MODIFICACIÓN
// ============================================
...
async function eliminarDireccionDeRuta(idDireccion) {
    if (!rutaActual) return;
...
function cancelarAccion() {
    if (estadoActual === ESTADOS.RUTA_EDITAR && rutaActual) {
        cambiarEstado(ESTADOS.RUTA_VER, {ruta: rutaActual});
    } else {
        cambiarEstado(ESTADOS.NEUTRO);
    }
}
...
function actualizarListaZonasEdicion() {
    ...
    rutaActual.zonas.forEach(zona => {
        ...
        item.querySelector('.btn-quitar-zona').onclick = async () => {
            if (confirm(`Remove zone "${zona.nombre_zona}" from route?\n\nThis will remove all its addresses from the route.`)) {
...
function actualizarIndicadorEstado(texto = null) {
    ...
    const textos = {
        [ESTADOS.NEUTRO]: {text: 'Ready', bg: '#95a5a6'},
        [ESTADOS.ADDRESSES]: {text: 'Viewing All Addresses', bg: '#9b59b6'},
        [ESTADOS.RUTA_VER]: {text: `Viewing: ${rutaActual?.nombre_ruta || 'Route'}`, bg: '#3498db'},
        [ESTADOS.RUTA_EDITAR]: {text: `Editing: ${rutaActual?.nombre_ruta || 'Route'}`, bg: '#f39c12'}
    };
    
    const config = textos[estadoActual] || {text: texto || 'Ready', bg: '#95a5a6'};
...
function zoomADireccion(idDireccion) {
    ...
    if (encontrada) {
        ...
        encontrada.setStyle({fillColor: '#ffff00', fillOpacity: 1, radius: 100});
        setTimeout(() => {
            encontrada.setStyle({fillColor: originalFill, fillOpacity: 0.7, radius: 75});
        }, 1500);
    }
}
