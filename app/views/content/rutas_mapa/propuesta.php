...
<script>
    ...
    // ============================================
    // NUEVO: ESTADO DE RUTA CARGADA (OPCIÓN A)
    // ============================================
    // Este estado representa: "Hay una ruta cargada en vista previa, 
    // lista para editar si el usuario lo desea"
    // modoEdicionRuta = null && rutaActual !== null → Estado ruta_cargada
    ...
    async function cargarDireccionesDeZonaenRutas(id_ruta) {
        try {
            ...
            if (todasLasDirecciones.length === 0) {
                ...
                // ============================================
                // NUEVO: ELIMINAR DIRECCIÓN CON CONFIRMACIÓN Y LÓGICA DE ZONA VACÍA
                // ============================================
                lista.querySelectorAll('.btn-eliminar-direccion').forEach(btn => {
                    btn.addEventListener('click', async (e) => {
                        e.stopPropagation();
                        const idDireccion = parseInt(btn.dataset.id);
                        
                        // ************** INICIO: AREA DE ELIMINAR DIRECCION **************
                        // PASO 1: Consultar backend para saber implicaciones de la eliminación
                        // Esta consulta debe retornar:
                        // - zona_quedara_vacia: boolean
                        // - total_direcciones_actuales_en_zona: number
                        // - id_zona: number
                        // - id_ruta: number
                        ...
                        try {
                            ...
                            // Consulta de pre-validación al backend
                            const checkResponse = await fetch(ruta_rutas_mapa_ajax, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    modulo_rutas: 'verificar_eliminacion_direccion',
                            ...
                            if (!checkData.success) {
                                throw new Error(checkData.error || 'Failed to check deletion impact');
                            }
                            
                            const { zona_quedara_vacia, nombre_zona, total_direcciones } = checkData.data;
                            
                            // PASO 2: Primera confirmación - ¿Eliminar dirección?
                            const confirmarEliminar = await suiteConfirm(
                                'Remove Address?',
                                `Are you sure you want to remove this address from the route?\n\nZone: ${nombre_zona}\nRemaining addresses after removal: ${total_direcciones - 1}`,
                            ...
                            // PASO 3: Segunda confirmación (condicional) - ¿Zona quedará vacía?
                            let eliminarZonaVacia = false;
                            
                            if (zona_quedara_vacia) {
                                const confirmarZona = await suiteConfirm(
                                    'Zone Will Be Empty',
                                    `Zone "${nombre_zona}" will have no addresses.\n\nDo you want to:\n• REMOVE zone from route calculation\n• KEEP zone for navigation purposes only`,
                                    { aceptar: 'Remove Zone', cancelar: 'Keep Zone' }
                                );
                                
                                // true = eliminar zona, false = mantener zona vacía
                                eliminarZonaVacia = confirmarZona;
                            }
                            
                            // PASO 4: Ejecutar eliminación definitiva
                            const deleteResponse = await fetch(ruta_rutas_mapa_ajax, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    modulo_rutas: 'eliminar_direccion_ruta',
                                    id_direccion: idDireccion,
                                    id_ruta: rutaActual.id_ruta,
                                    eliminar_zona_vacia: eliminarZonaVacia
                                    // Backend debe:
                                    // 1. DELETE FROM rutas_direcciones WHERE id_direccion = X
                                    // 2. IF eliminar_zona_vacia = true:
                                    //    DELETE FROM rutas_zonas_cuadricula WHERE id_zona = Y AND id_ruta = Z
                                    // 3. UPDATE CONTRATOS SET id_ruta = NULL WHERE id_direccion = X
                                    //    (o el campo que relacione contrato con dirección)
                                    // 4. Retornar success con detalle de operaciones realizadas
                                })
                            });
                            ...
                            if (!deleteData.success) {
                                throw new Error(deleteData.error || 'Failed to delete address');
                            }
                            
                            suiteLoading('hide');
                            
                            // Mostrar resumen de operación
                            let mensajeExito = 'Address removed successfully.';
                            if (zona_quedara_vacia && eliminarZonaVacia) {
                                mensajeExito += `\nEmpty zone "${nombre_zona}" removed from route.`;
                            } else if (zona_quedara_vacia && !eliminarZonaVacia) {
                                mensajeExito += `\nZone "${nombre_zona}" kept for navigation (no addresses).`;
                            }
                            
                            await suiteAlertSuccess('Done!', mensajeExito);
                        ...
                        // ************** FIN: AREA DE ELIMINAR DIRECCION **************
                    });
                });
            }
    ...
    // ============================================
    // NUEVO: ACTUALIZAR ESTADO INTERFAZ CON OPCIÓN A
    // ============================================
    function actualizarEstadoInterfaz() {
        ...
        try {
            ...
            // ============================================
            // NUEVO: LÓGICA DE ESTADO RUTA_CARGADA (OPCIÓN A)
            // ============================================
            // Estado especial: Hay ruta cargada pero no estamos editando
            const estadoRutaCargada = (modoEdicionRuta === null && rutaActual !== null);
            ...
            if (modoEdicionRuta === 'crear') {
                ...
                modoIndicador.style.padding = '6px';
                modoIndicador.style.borderRadius = '4px';
            } else if (estadoRutaCargada) {
                // NUEVO: Estado ruta_cargada - Vista previa de ruta existente
                modoIndicador.textContent = '📍 Route Loaded - Ready to Edit';
                modoIndicador.style.backgroundColor = '#e8f4f8';
                modoIndicador.style.padding = '6px';
                modoIndicador.style.borderRadius = '4px';
            } else {
            ...
            // ============================================
            // NUEVO: VISIBILIDAD DE BOTONES CON ESTADO RUTA_CARGADA
            // ============================================
            if (modoEdicionRuta === 'crear' || modoEdicionRuta === 'editar') {
                // Modo edición activa: Mostrar Cancelar, Actualizar (como Save), ocultar Crear
                btnCancelar.style.display = 'block';
                btnCrear.style.display = 'none';
                btnActualizar.style.display = 'block';
                btnEliminar.style.display = 'block';
                
                // En modo editar, el botón Actualizar actúa como "Save Changes"
                if (modoEdicionRuta === 'editar') {
                    btnActualizar.textContent = '💾 Save Changes';
                    btnActualizar.style.backgroundColor = '#27ae60';
                }
            } else if (estadoRutaCargada) {
                // NUEVO: Estado ruta_cargada - Mostrar Actualizar y Eliminar, ocultar Crear y Cancelar
                btnCrear.style.display = 'none';
                btnCancelar.style.display = 'none';
                btnActualizar.style.display = 'block';
                btnEliminar.style.display = 'block';
                
                // Botón Actualizar en modo "entrar a editar"
                btnActualizar.textContent = 'Update This Route';
                btnActualizar.style.backgroundColor = '#f39c12';
            } else {
                // Estado neutral: Solo mostrar Crear
                btnCancelar.style.display = 'none';
            ...
            const tryInit = function () {
                ...
                document.getElementById('btn-actualizar-ruta').addEventListener('click', function () {
                    ...
                    if (modoVisualizacion === MODO_VISUALIZACION.VER) {
                        ...
                        habilitarMapa(); // Esto debería activar los clicks en zonas
    ...
    async function cargarZonasRuta(id_ruta) {
        ...
        try {
            ...
            // ============================================
            // NUEVO: ACTUALIZAR ESTADO INTERFAZ (OPCIÓN A)
            // Esto activará el estado ruta_cargada y mostrará Update/Delete
            // ============================================
            actualizarEstadoInterfaz();
