...
$pagina_address_clas = $pagina;

$proceso_actual = "address_clas";
$busqueda = "";
...
$ruta_address_clasnew = "address_clasNew";
...
if (!isset($address_clas)) {
    echo "Problema con el Controlador de Address Class";
}

if (!isset($_SESSION['nav_address_clas'])) {
    // Almacenar el nivel y página actual en la sesión
    $_SESSION['nav_address_clas'] = [
        'pagina_address_clas' => $pagina_address_clas,
        'registrosPorPagina' => $registrosPorPagina
    ];
} else {
    if (isset($_SESSION['nav_address_clas']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_address_clas']['registrosPorPagina'])) {
            $_SESSION['nav_address_clas']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_address_clas']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_address_clas']['orden'])) {
        $f_orden = $_SESSION['nav_address_clas']['orden'];

        $ordenStr = $_SESSION['nav_address_clas']['orden'];
    ...
    if (isset($_SESSION['nav_address_clas']['pagina_address_clas'])) {
        if (!is_numeric($_SESSION['nav_address_clas']['pagina_address_clas'])) {
            $_SESSION['nav_address_clas']['pagina_address_clas'] = $pagina_address_clas;
        }
    } else {
        $_SESSION['nav_address_clas'] = [
            'pagina_address_clas' => $pagina_address_clas
...
$no_hacer = true;
$ruta_address_clas_ajax = APP_URL . "/app/ajax/address_clasAjax.php";
...
<script type="text/javascript">
    ...
    function actualizarVista() {
        ...
        const estado_address_clas = document.querySelector('input[name="estado_address_clas"]:checked')?.value || 'all';

        const formData = new FormData();
        formData.append('modulo_address_clas', 'actualizar_navegacion');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const radiosEstado = document.querySelectorAll('input[name="estado_address_clas"]');
        ...
        if (ordenContainer && window.ordenActivoPHP) {
            const ordenCampos = window.ordenActivoPHP; // ['piso', 'address_clas', ...]
