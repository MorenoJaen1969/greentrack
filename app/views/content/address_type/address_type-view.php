...
$pagina_address_type = $pagina;

$proceso_actual = "address_type";
$busqueda = "";
...
$ruta_address_typenew = "address_typeNew";
...
if (!isset($address_type)) {
    echo "Problema con el Controlador de Address Type";
}

if (!isset($_SESSION['nav_address_type'])) {
    // Almacenar el nivel y página actual en la sesión
    $_SESSION['nav_address_type'] = [
        'pagina_address_type' => $pagina_address_type,
        'registrosPorPagina' => $registrosPorPagina
    ];
} else {
    if (isset($_SESSION['nav_address_type']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_address_type']['registrosPorPagina'])) {
            $_SESSION['nav_address_type']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_address_type']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_address_type']['orden'])) {
        $f_orden = $_SESSION['nav_address_type']['orden'];

        $ordenStr = $_SESSION['nav_address_type']['orden'];
    ...
    if (isset($_SESSION['nav_address_type']['pagina_address_type'])) {
        if (!is_numeric($_SESSION['nav_address_type']['pagina_address_type'])) {
            $_SESSION['nav_address_type']['pagina_address_type'] = $pagina_address_type;
        }
    } else {
        $_SESSION['nav_address_type'] = [
            'pagina_address_type' => $pagina_address_type
...
$no_hacer = true;
$ruta_address_type_ajax = APP_URL . "/app/ajax/address_typeAjax.php";
...
<script type="text/javascript">
    ...
    function actualizarVista() {
        ...
        const estado_address_type = document.querySelector('input[name="estado_address_type"]:checked')?.value || 'all';

        const formData = new FormData();
        formData.append('modulo_address_type', 'actualizar_navegacion');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const radiosEstado = document.querySelectorAll('input[name="estado_address_type"]');
        ...
        if (ordenContainer && window.ordenActivoPHP) {
            const ordenCampos = window.ordenActivoPHP; // ['piso', 'address_type', ...]
