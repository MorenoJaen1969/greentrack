...
$pagina_dias_no_actividad = $pagina;

$proceso_actual = "dias_no_actividad";
$busqueda = "";
...
$ruta_dias_no_actividadnew = "dias_no_actividadNew";
...
if (!isset($dias_no_actividad)) {
    echo "Problema con el Controlador de Address Class";
}

if (!isset($_SESSION['nav_dias_no_actividad'])) {
    // Almacenar el nivel y página actual en la sesión
    $_SESSION['nav_dias_no_actividad'] = [
        'pagina_dias_no_actividad' => $pagina_dias_no_actividad,
        'registrosPorPagina' => $registrosPorPagina
    ];
} else {
    if (isset($_SESSION['nav_dias_no_actividad']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_dias_no_actividad']['registrosPorPagina'])) {
            $_SESSION['nav_dias_no_actividad']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_dias_no_actividad']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_dias_no_actividad']['orden'])) {
        $f_orden = $_SESSION['nav_dias_no_actividad']['orden'];

        $ordenStr = $_SESSION['nav_dias_no_actividad']['orden'];
    ...
    if (isset($_SESSION['nav_dias_no_actividad']['pagina_dias_no_actividad'])) {
        if (!is_numeric($_SESSION['nav_dias_no_actividad']['pagina_dias_no_actividad'])) {
            $_SESSION['nav_dias_no_actividad']['pagina_dias_no_actividad'] = $pagina_dias_no_actividad;
        }
    } else {
        $_SESSION['nav_dias_no_actividad'] = [
            'pagina_dias_no_actividad' => $pagina_dias_no_actividad
...
$no_hacer = true;
$ruta_dias_no_actividad_ajax = APP_URL . "/app/ajax/dias_no_actividadAjax.php";
$encabezado = PROJECT_ROOT . "/app/views/inc/encabezado.php";
$opcion = "dias_no_actividad";
?>
    ...
    <div class="container pb-1 pt-1">
        <div id="datos_act" name="datos_act">
            <?php
                ...
                echo $dias_no_actividad->listardias_no_actividadControlador($param_datos);
            ?>
        </div>
    </div>
</main>
...
<script type="text/javascript">
    ...
    function actualizarVista() {
        ...
        const estado_dias_no_actividad = document.querySelector('input[name="estado_dias_no_actividad"]:checked')?.value || 'all';

        const formData = new FormData();
        formData.append('modulo_dias_no_actividad', 'actualizar_navegacion');
        ...
        formData.append('f_estado', estado_dias_no_actividad);
        formData.append('orden', ordenStr);

        const ruta_dias_no_actividad_ajax = "<?php echo $ruta_dias_no_actividad_ajax; ?>";

        // === 4. Enviar vía fetch ===
        fetch(ruta_dias_no_actividad_ajax, {
            method: 'POST',
            body: formData
        })
            .then(response => {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const radiosEstado = document.querySelectorAll('input[name="estado_dias_no_actividad"]');
        ...
        if (ordenContainer && window.ordenActivoPHP) {
            const ordenCampos = window.ordenActivoPHP; // ['piso', 'dias_no_actividad', ...]
        ...
        // Cambio: edificio
        if (cmb_edificios) {
            cmb_edificios.addEventListener('change', actualizarVista);
        }

        // Cambio: piso
        if (cmb_pisos) {
            cmb_pisos.addEventListener('change', actualizarVista);
        }

        if (btnAplicarOrden) {
            btnAplicarOrden.addEventListener('click', actualizarVista);
        }
