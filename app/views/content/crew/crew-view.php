...
if (!isset($crew)) {
    echo "Problema con el Controlador de Crew";
}

if (!isset($_SESSION['nav_crew'])) {
    // Almacenar el nivel y página actual en la sesión
    $_SESSION['nav_crew'] = [
        'pagina_crew' => $pagina_crew,
        'registrosPorPagina' => $registrosPorPagina 
    ];
} else {
    if (isset($_SESSION['nav_crew']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_crew']['registrosPorPagina'])) {
            $_SESSION['nav_crew']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_crew']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_crew']['orden'])) {
        $f_orden = $_SESSION['nav_crew']['orden'];

        $ordenStr = $_SESSION['nav_crew']['orden'];
    ...
    if (isset($_SESSION['nav_crew']['pagina_crew'])) {
        if (!is_numeric($_SESSION['nav_crew']['pagina_crew'])) {
            $_SESSION['nav_crew']['pagina_crew'] = $pagina_crew;
        }
    } else {
        $_SESSION['nav_crew'] = [
            'pagina_crew' => $pagina_crew
...
$no_hacer = true;
$ruta_crew_ajax = APP_URL . "/app/ajax/crewAjax.php";
    ...
    <div class="container pb-1 pt-1">
        <div id="datos_act" name="datos_act">
            <?php
                ...
                echo $crew->listarcrewControlador($param_datos);
            ?>
        </div>
    </div>
</main>
...
window.CONFIG_BUSQUEDA = <?php echo json_encode([
    'modulo' => $proceso_actual ?? 'crew',
    'pagina' => $pagina_crew ?? 1,
...
<script type="text/javascript">
    ...
    // === Eliminación segura de crew con suiteConfirm ===
    document.addEventListener('click', async function(e) {
        const btn = e.target.closest('.btn-eliminar-crew');
        if (!btn) return;
        ...
        try {
            const res = await fetch('/app/ajax/crewAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_crew: 'eliminar',
            ...
            if (data.success) {
                ...
                recargarTablacrew(pagina, registrosPorPagina, urlOrigen, valor);
    ...
    async function recargarTablacrew(pagina, registrosPorPagina, urlOrigen, busqueda = '') {
        try {
            const res = await fetch('/app/ajax/crewAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_crew: 'listar_tabla',
                    pagina: pagina,
            ...
            const wrapper = document.getElementById('tabla-crew-wrapper');
            if (wrapper) {
                wrapper.outerHTML = html;
            } else {
                console.error('❌ No se encontró #tabla-crew-wrapper en el DOM');
            }
        } catch (err) {
            console.error('Error al recargar tabla de crew:', err);
