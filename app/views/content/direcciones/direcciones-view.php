...
$pagina_direcciones = $pagina;

$proceso_actual = "direcciones";
$busqueda = "";
...
$ruta_direccionesnew = "direccionesNew";
...
if (!isset($_SESSION['nav_direcciones'])) {
    // Almacenar el nivel y página actual en la sesión
    $_SESSION['nav_direcciones'] = [
        'pagina_direcciones' => $pagina_direcciones,
        'registrosPorPagina' => $registrosPorPagina 
    ];
} else {
    if (isset($_SESSION['nav_direcciones']['registrosPorPagina'])) {
        if (!is_numeric($_SESSION['nav_direcciones']['registrosPorPagina'])) {
            $_SESSION['nav_direcciones']['registrosPorPagina'] = $registrosPorPagina;
        }
        $registrosPorPagina = $_SESSION['nav_direcciones']['registrosPorPagina'];
    }

    if (isset($_SESSION['nav_direcciones']['orden'])) {
        $f_orden = $_SESSION['nav_direcciones']['orden'];

        $ordenStr = $_SESSION['nav_direcciones']['orden'];
    ...
    if (isset($_SESSION['nav_direcciones']['pagina_direcciones'])) {
        if (!is_numeric($_SESSION['nav_direcciones']['pagina_direcciones'])) {
            $_SESSION['nav_direcciones']['pagina_direcciones'] = $pagina_direcciones;
        }
    } else {
        $_SESSION['nav_direcciones'] = [
            'pagina_direcciones' => $pagina_direcciones
...
$no_hacer = true;
$ruta_direcciones_ajax = APP_URL . "/app/ajax/direccionesAjax.php";
...
<script type="text/javascript">
    ...
    // === Eliminación segura de direcciones con suiteConfirm ===
    document.addEventListener('click', async function(e) {
        ...
        try {
            const res = await fetch('/app/ajax/direccionesAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_direcciones: 'eliminar',
            ...
            if (data.success) {
                ...
                recargarTablaDirecciones(pagina, registrosPorPagina, urlOrigen, valor);
    ...
    async function recargarTablaDirecciones(pagina, registrosPorPagina, urlOrigen, busqueda = '') {
        try {
            const res = await fetch('/app/ajax/direccionesAjax.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    modulo_direcciones: 'listar_tabla',
                    pagina: pagina,
            ...
            const wrapper = document.getElementById('tabla-direcciones-wrapper');
            if (wrapper) {
                wrapper.outerHTML = html;
            } else {
                console.error('❌ No se encontró #tabla-direcciones-wrapper en el DOM');
            }
        } catch (err) {
            console.error('Error al recargar tabla de direcciones:', err);
