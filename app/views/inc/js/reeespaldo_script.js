    ...
    async function cargarMatrizDiaria() {
        ...
        try {
            ...
            clientes.forEach(cliente => {
                ...
                // ✅ OBTENER COLOR DE RUTA PARA ESTE CLIENTE
                // Buscamos el primer servicio de este cliente para obtener su color_ruta
                const servicioCliente = servicios.find(s => s.cliente === cliente);
                const colorRutaOriginal = servicioCliente?.color_ruta || '#f5f5f5';
                const colorRutaTenue = suavizarColor(colorRutaOriginal, 0.25); // 25% color, 75% blanco

                // ✅ Aplicar color tenue al tdCliente (manteniendo tus estilos originales)
                tdCliente.style.cssText = `
                    font-weight: bold; 
                    background: ${colorRutaTenue}; 
                ...
                // ✅ Efecto hover opcional para el cliente (un poco más oscuro)
                tdCliente.addEventListener('mouseenter', () => {
                    tdCliente.style.backgroundColor = suavizarColor(colorRutaOriginal, 0.4);
                });
                tdCliente.addEventListener('mouseleave', () => {
                    tdCliente.style.backgroundColor = colorRutaTenue;
                });
                ...
                crews.forEach((crew, idx) => {
                    ...
                    // ✅ OBTENER COLOR DE RUTA (asumiendo que crew tiene color_ruta)
                    const colorRutaOriginal = crew.color_ruta || '#f5f5f5';
                    const colorRutaTenue = suavizarColor(colorRutaOriginal, 0.25); // 25% color, 75% blanco
                    // O usa transparencia: const colorRutaTenue = colorConTransparencia(colorRutaOriginal, 0.2);
                    
                    td.style.cssText = `
                        border: 1px solid #ddd; 
                        padding: 6px; 
                        text-align: center;
                        background-color: ${colorRutaTenue};
                        transition: background-color 0.2s ease;
                    `;

                    // Efecto hover opcional (un poco más oscuro)
                    td.addEventListener('mouseenter', () => {
                        td.style.backgroundColor = suavizarColor(colorRutaOriginal, 0.4);
                    });
                    td.addEventListener('mouseleave', () => {
                        td.style.backgroundColor = colorRutaTenue;
                    });
