...
// === 7. Procesar módulo ===
$modulo = $inputData['modulo_facturacion'] ?? '';
...
switch ($modulo) {
    case 'generar_facturacion':
        ...
        $controller->generarFacturasPendientes($id_contrato);
        http_response_code(200);
        echo json_encode(
            [
                'success' => true,
                'mess' => 'The billing was verified correctly.'
            ]
        );
        exit();

    case 'distribucion_facturacion':
        $id_contrato = $inputData['id_contrato'];
