...
// === 7. Procesar módulo ===
$modulo = $inputData['modulo_landuse'] ?? '';

if (!$modulo) {
    http_response_code(400);
    echo json_encode(['error' => 'The parameter is missing. "modulo_landuse"']);
    exit();
}

// === 8. Cargar el controlador ===
require_once  '../controllers/landUseController.php';

use app\controllers\landUseController;

$controller = new landUseController();

switch ($modulo) {
    case 'listar_select':
        echo json_encode($controller->consultar_land_use());
        break;
    ...
    case 'data_direcciones':
        $landuse = $controller->getlanduse();

        if (is_array($landuse)) {
            http_response_code(200);
            echo json_encode(['success' => true, 'data' => $landuse]);
        } else {
            http_response_code(400);
            echo json_encode(['error' => 'Query error: ' . $landuse]);
        }
        exit();
